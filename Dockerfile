...
WORKDIR /root
ADD https://github.com/kmatheussen/radium/archive/refs/tags/6.9.99.tar.gz .
RUN tar xf 6.9.99.tar.gz
RUN mkdir SDKs
WORKDIR /root/SDKs
...
WORKDIR /root/radium-6.9.99/bin/packages/
RUN sed -i "112 a     make -j`nproc` || echo \"retrying\"" build.sh
WORKDIR /root/radium-6.9.99
from setup as build
RUN make packages
